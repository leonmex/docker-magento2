...
class Interceptor extends \Magento\CatalogImportExport\Model\Import\Proxy\Product\ResourceModel implements \Magento\Framework\Interception\InterceptorInterface
{
     ...
     * {@inheritdoc}
     */
    public function addAttributeByScope(\Magento\Eav\Model\Entity\Attribute\AbstractAttribute $attribute, $suffix)
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'addAttributeByScope');
        if (!$pluginInfo) {
            return parent::addAttributeByScope($attribute, $suffix);
        } else {
            return $this->___callPlugins('addAttributeByScope', func_get_args(), $pluginInfo);
        }
    }
